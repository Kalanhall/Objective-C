...
NSUInteger const KLRSAKeySizeInBits2048 = 1 << 11;
NSUInteger const KLRSAKeySizeInBits1024 = 1 << 10;
...
@implementation KLRSA
static KLRSA *sharedKLRSA = nil;
+ (nonnull instancetype)sharedKLRSA {
	static dispatch_once_t onceToken;
	dispatch_once(&onceToken, ^{
		sharedKLRSA = [[super allocWithZone:NULL] init];
		sharedKLRSA.padding = kSecPaddingPKCS1SHA1;
		[sharedKLRSA generateKeys];
	});
	return sharedKLRSA;
}
+ (id)allocWithZone:(struct _NSZone *)zone {
	return [KLRSA sharedKLRSA];
}
...
- (BOOL)generateKeys {
	return [self generateKeys:KLRSAKeySizeInBits2048];
}

- (BOOL)generateKeys:(NSUInteger)keySizeInBits {
	NSParameterAssert(keySizeInBits == KLRSAKeySizeInBits2048 || keySizeInBits == KLRSAKeySizeInBits1024);
...
//		}
		...
		{
			digest = [data kl_SHA224];
			break;
		}
		case kSecPaddingPKCS1SHA256:
		{
			digest = [data kl_SHA256];
			break;
		}
		case kSecPaddingPKCS1SHA384:
		{
			digest = [data kl_SHA384];
			break;
		}
		case kSecPaddingPKCS1SHA512:
		{
			digest = [data kl_SHA512];
			break;
		}
		default:
        {
...
//		{
//			digest = [data kl_MD2];/* Unsupported as of iOS 5.0 */
//			break;
//		}
//		case kSecPaddingPKCS1MD5:
//		{
//			digest = [data kl_MD5];/* Unsupported as of iOS 5.0 */
//			break;
//		}
		case kSecPaddingPKCS1SHA1:
		{
