...
NS_INLINE BOOL KLEqualIndexSection(KLIndexSection indexSection1,KLIndexSection indexSection2) {
    return indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section;
}

NS_INLINE KLIndexSection KLMakeIndexSection(NSInteger index, NSInteger section) {
    KLIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface KLCarouselView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, KLCarouselTransformLayoutDelegate> {
    struct {
    ...
    struct {
        ...
        unsigned int layoutForCarouselView   :1;
    }_dataSourceFlags;
}
...
@property (nonatomic, assign) KLIndexSection beginDragIndexSection;
...
#define kCarouselViewMaxSectionCount 200
#define kCarouselViewMinSectionCount 18
...
- (void)addCollectionView {
    KLCarouselTransformLayout *layout = [[KLCarouselTransformLayout alloc]init];
...
- (void)timerFired:(NSTimer *)timer {
    ...
    [self scrollToNearlyIndexAtDirection:KLPagerScrollDirectionRight animate:YES];
}

#pragma mark - getter

- (KLCarouselViewLayout *)layout {
    if (!_layout) {
        if (_dataSourceFlags.layoutForCarouselView) {
            _layout = [_dataSource layoutForCarouselView:self];
...
- (void)setDelegate:(id<KLCarouselViewDelegate>)delegate {
    _delegate = delegate;
    _delegateFlags.carouselViewDidScroll = [delegate respondsToSelector:@selector(carouselViewDidScroll:)];
    _delegateFlags.didScrollFromIndexToNewIndex = [delegate respondsToSelector:@selector(carouselView:didScrollFromIndex:toIndex:)];
    _delegateFlags.initializeTransformAttributes = [delegate respondsToSelector:@selector(carouselView:initializeTransformAttributes:)];
    _delegateFlags.applyTransformToAttributes = [delegate respondsToSelector:@selector(carouselView:applyTransformToAttributes:)];
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((KLCarouselTransformLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<KLCarouselViewDataSource>)dataSource {
    _dataSource = dataSource;
    _dataSourceFlags.cellForItemAtIndex = [dataSource respondsToSelector:@selector(carouselView:cellForItemAtIndex:)];
    _dataSourceFlags.layoutForCarouselView = [dataSource respondsToSelector:@selector(layoutForCarouselView:)];
}
...
- (void)updateData {
    [self updateLayout];
    _numberOfItems = [_dataSource numberOfItemsInCarouselView:self];
    ...
    [self resetCarouselViewAtIndex:(_indexSection.index < 0 && !CGRectIsEmpty(self.collectionView.frame)) || needResetIndex ? 0 :_indexSection.index];
    if (needResetIndex) {
        [self addTimer];
    }
}
...
- (void)scrollToNearlyIndexAtDirection:(KLPagerScrollDirection)direction animate:(BOOL)animate {
    KLIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
...
- (void)scrollToItemAtIndex:(NSInteger)index animate:(BOOL)animate {
    ...
    if (!_isInfiniteLoop) {
        [self scrollToItemAtIndexSection:KLMakeIndexSection(index, 0) animate:animate];
        return;
    }

    [self scrollToItemAtIndexSection:KLMakeIndexSection(index, index >= self.curIndex ? _indexSection.section : _indexSection.section+1) animate:animate];
}
...
- (void)scrollToItemAtIndexSection:(KLIndexSection)indexSection animate:(BOOL)animate {
    ...
    if (animate && [_delegate respondsToSelector:@selector(carouselViewWillBeginScrollingAnimation:)]) {
...
- (void)updateLayout {
    ...
    ((KLCarouselTransformLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (void)setNeedUpdateLayout {
    ...
    [self resetCarouselViewAtIndex:_indexSection.index < 0 ? 0 :_indexSection.index];
}

#pragma mark - pager index

- (BOOL)isValidIndexSection:(KLIndexSection)indexSection {
    return indexSection.index >= 0 && indexSection.index < _numberOfItems && indexSection.section >= 0 && indexSection.section < kCarouselViewMaxSectionCount;
}

- (KLIndexSection)nearlyIndexPathAtDirection:(KLPagerScrollDirection)direction{
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (KLIndexSection)nearlyIndexPathForIndexSection:(KLIndexSection)indexSection direction:(KLPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }
    
    if (!_isInfiniteLoop) {
        if (direction == KLPagerScrollDirectionRight && indexSection.index == _numberOfItems - 1) {
            return _autoScrollInterval > 0 ? KLMakeIndexSection(0, 0) : indexSection;
        } else if (direction == KLPagerScrollDirectionRight) {
            return KLMakeIndexSection(indexSection.index+1, 0);
        }
        
        if (indexSection.index == 0) {
            return _autoScrollInterval > 0 ? KLMakeIndexSection(_numberOfItems - 1, 0) : indexSection;
        }
        return KLMakeIndexSection(indexSection.index-1, 0);
    }
    
    if (direction == KLPagerScrollDirectionRight) {
        if (indexSection.index < _numberOfItems-1) {
            return KLMakeIndexSection(indexSection.index+1, indexSection.section);
        }
        if (indexSection.section >= kCarouselViewMaxSectionCount-1) {
            return KLMakeIndexSection(indexSection.index, kCarouselViewMaxSectionCount-1);
        }
        return KLMakeIndexSection(0, indexSection.section+1);
    }
    ...
    if (indexSection.section <= 0) {
        return KLMakeIndexSection(indexSection.index, 0);
    }
    return KLMakeIndexSection(_numberOfItems-1, indexSection.section-1);
}

- (KLIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
        return KLMakeIndexSection(0, 0);
    }
    ...
    if (middleOffset - leftEdge >= 0) {
        ...
        if (itemIndex < 0) {
            itemIndex = 0;
        }else if (itemIndex >= _numberOfItems*kCarouselViewMaxSectionCount) {
            itemIndex = _numberOfItems*kCarouselViewMaxSectionCount-1;
        }
        curIndex = itemIndex%_numberOfItems;
        curSection = itemIndex/_numberOfItems;
    }
    return KLMakeIndexSection(curIndex, curSection);
}

- (CGFloat)caculateOffsetXAtIndexSection:(KLIndexSection)indexSection{
    if (_numberOfItems == 0) {
        return 0;
    }
    UICollectionViewFlowLayout *layout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
...
- (void)resetCarouselViewAtIndex:(NSInteger)index {
    ...
    [self scrollToItemAtIndexSection:KLMakeIndexSection(index, _isInfiniteLoop ? kCarouselViewMaxSectionCount/3 : 0) animate:NO];
    if (!_isInfiniteLoop && _indexSection.index < 0) {
        [self scrollViewDidScroll:_collectionView];
    }
}

- (void)recycleCarouselViewIfNeed {
    if (!_isInfiniteLoop) {
        return;
    }
    if (_indexSection.section > kCarouselViewMaxSectionCount - kCarouselViewMinSectionCount || _indexSection.section < kCarouselViewMinSectionCount) {
        [self resetCarouselViewAtIndex:_indexSection.index];
    }
}
...
- (NSInteger)numberOfSectionsInCollectionView:(UICollectionView *)collectionView {
    return _isInfiniteLoop ? kCarouselViewMaxSectionCount : 1;
}
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_dataSourceFlags.cellForItemAtIndex) {
       return [_dataSource carouselView:self cellForItemAtIndex:indexPath.row];
    }
    NSAssert(NO, @"carouselView cellForItemAtIndex: is nil!");
    return nil;
}
...
- (UIEdgeInsets)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout insetForSectionAtIndex:(NSInteger)section {
    ...
    }else if (section == kCarouselViewMaxSectionCount -1) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    KLIndexSection newIndexSection =  [self caculateIndexSectionWithOffsetX:scrollView.contentOffset.x];
...
//        NSLog(@"inVlaidIndexSection:(%ld,%ld)!",(long)newIndexSection.index,(long)newIndexSection.section);
    ...
    KLIndexSection indexSection = _indexSection;
    _indexSection = newIndexSection;
    
    if (_delegateFlags.carouselViewDidScroll) {
        [_delegate carouselViewDidScroll:self];
    }
    
    if (_delegateFlags.didScrollFromIndexToNewIndex && !KLEqualIndexSection(_indexSection, indexSection)) {
        //NSLog(@"curIndex %ld",(long)_indexSection.index);
        [_delegate carouselView:self didScrollFromIndex:MAX(indexSection.index, 0) toIndex:_indexSection.index];
    }
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    if ([_delegate respondsToSelector:@selector(carouselViewWillBeginDragging:)]) {
        [_delegate carouselViewWillBeginDragging:self];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if (fabs(velocity.x) < 0.35 || !KLEqualIndexSection(_beginDragIndexSection, _indexSection)) {
        targetContentOffset->x = [self caculateOffsetXAtIndexSection:_indexSection];
        return;
    }
    KLPagerScrollDirection direction = KLPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = KLPagerScrollDirectionLeft;
    }
    KLIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if ([_delegate respondsToSelector:@selector(carouselViewDidEndDragging:willDecelerate:)]) {
        [_delegate carouselViewDidEndDragging:self willDecelerate:decelerate];
    }
}

- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView {
    if ([_delegate respondsToSelector:@selector(carouselViewWillBeginDecelerating:)]) {
        [_delegate carouselViewWillBeginDecelerating:self];
    }
}

- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    [self recycleCarouselViewIfNeed];
    if ([_delegate respondsToSelector:@selector(carouselViewDidEndDecelerating:)]) {
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView {
    [self recycleCarouselViewIfNeed];
    if ([_delegate respondsToSelector:@selector(carouselViewDidEndScrollingAnimation:)]) {
        [_delegate carouselViewDidEndScrollingAnimation:self];
    }
}

#pragma mark - KLCarouselTransformLayoutDelegate
...
- (void)carouselViewTransformLayout:(KLCarouselTransformLayout *)carouselViewTransformLayout initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes {
    if (_delegateFlags.initializeTransformAttributes) {
        [_delegate carouselView:self initializeTransformAttributes:attributes];
    }
}
...
- (void)carouselViewTransformLayout:(KLCarouselTransformLayout *)carouselViewTransformLayout applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes {
    if (_delegateFlags.applyTransformToAttributes) {
        [_delegate carouselView:self applyTransformToAttributes:attributes];
    }
}
...
- (void)dealloc {
    ((KLCarouselTransformLayout *)_collectionView.collectionViewLayout).delegate = nil;
